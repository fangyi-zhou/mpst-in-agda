...
force (∞project g r) = project (force g) r
...
proj-prefix-other :
  ...
  -> r ≢ t
  -> s ≢ t
  -> project (msgSingle r s r≢s l gSub) t ≡ force (∞project gSub t)
...
proj-inv-send :
  ...
  -> ∃[ p≢q ] ∃[ gSub ] g ≡ msgSingle p q p≢q l gSub × ∞project gSub p ≡ lSub
     ...
     ∃[ r ] ∃[ s ] ∃[ r≢s ] ∃[ l′ ] ∃[ gSub ]
       ...
       r ≢ p ×
       s ≢ p ×
       force (∞project gSub p) ≡ sendSingle q l lSub
...
... | no r≢p   | no s≢p   = inj₂ (r , s , r≢s , l′ , gSub , refl , r≢p , s≢p , {!   !})
... | yes refl | no s≢p   = {!   !}
...
proj-inv-recv :
  ...
  -> ∃[ p≢q ] ∃[ gSub ] g ≡ msgSingle q p p≢q l gSub × ∞project gSub p ≡ lSub
     ...
     ∃[ r ] ∃[ s ] ∃[ r≢s ] ∃[ l′ ] ∃[ gSub ]
       ...
       r ≢ p ×
       s ≢ p ×
       force (∞project gSub p) ≡ recvSingle q l lSub
...
proj-inv-send-recv :
  ...
  -> ∃[ p≢q ] ∃[ gSub ] g ≡ msgSingle p q p≢q l gSub × ∞project gSub p ≡ lpSub × ∞project gSub q ≡ lqSub
     ...
     ∃[ r ] ∃[ s ] ∃[ r≢s ] ∃[ l′ ] ∃[ gSub ]
       ...
       r ≢ q ×
       s ≢ q ×
       force (∞project gSub p) ≡ sendSingle q l lpSub ×
       force (∞project gSub q) ≡ recvSingle p l lqSub
...
config-gt-remove-prefix :
  ...
  -> ∃[ cSub ] cSub ≡ (c [ p ]≔ force (∞project gSub p)) [ q ]≔ force (∞project gSub q) × force gSub ↔ cSub
config-gt-remove-prefix {n} {p} {q} {_} {p≢q} {gSub} g c assoc refl
  = {!   !}
  -- = cSub , refl , (record { isProj = isProj-gSub })
  --   where
  --     lpSub = force (∞project gSub p)
  --     lqSub = force (∞project gSub q)
  --     cSub = (c [ p ]≔ lpSub) [ q ]≔ lqSub
  --     isProj-gSub : ∀ (r : Fin n) -> lookup cSub r ≡ force (∞project gSub r)
  --     isProj-gSub r
  --       with p ≟ r    | q ≟ r
  --     ... | yes refl | yes refl = ⊥-elim (p≢q refl)
  --     ... | yes refl | no  _
